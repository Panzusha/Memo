...
class RegisterController extends Controller
{
    ...
    public function register(Request $request): RedirectResponse
    {
        ...
        // si tous les champs sont validés, données stockées dans le tableau $validated
        $validated = $request->validate([
            // between = nb de caratères
            'name' => ['required', 'string', 'between:5,255'],
            // unique : viendra verifier la colonne email dans table users
            'email' => ['required', 'email', 'unique:users'],
            // confirmed indique la présence d'un champ de confirmation (qui sera régi par les mêmes critères)
        ...
        // utilisation session flash (mot clé with)  voir doc laravel et prise de notes page 7
